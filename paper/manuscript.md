...
**Candour note.** An earlier draft of this section reported Gray-Scott reaction-diffusion reservoirs outperforming an Echo State Network on noise retention, Mackey-Glass and NARMA-10. Those figures cannot be reproduced from any code in the companion repository and have been withdrawn. Everything reported below is generated by `benchmark_v050.py` in `biofield_sim` v0.5.0 and written to `benchmark_results_v050.json`; the reader can regenerate every number in under ten seconds. Below, we first report diagnostic baselines on continuous reservoir substrates generated by `benchmark_v050.py` in `biofield_sim` v0.5.0 (§6.1), which are negative at the parameters tested. We then report the pre-registered wipe-resumption benchmark implemented in v0.6.0 (`benchmark_wipe_v060.py`, 30 seeds) in §6.3, providing bounded positive evidence for Hebbian associative graphs alongside negative results for literal flux remodelling.
